...
class Runner(base_runner.BaseRunner):
    ...
    def _setup_data_log(self, config):
        ...
        dist_hist = np.zeros((config.num_seeds, config.num_episodes, config.num_arms))
        ...
        stimulations = np.zeros(scalar_data_shape)
        ...
        for seed in range(config.num_seeds):
            for episode in range(config.num_episodes):
                ...
                if not change:
                    ...
                    seed_ep_dists = [
                        scipy.stats.bernoulli(probs[a]) for a in range(config.num_arms)
                    ]
        ...
        for episode in range(num_episodes):
            ...
            for i, name in enumerate(agent_order):
                ...
                for trial in range(change_frequency):
                    ...
                    if episode != 0 and trial == int(0.25 * change_frequency):
                        agent.toggle_stimulation()

                    if trial == int(0.75 * change_frequency):
                        agent.toggle_stimulation()
                    ...
                    # print(
                    #   action, arm_sample, dist[best_arm].mean(), dist[action].mean()
                    # )
                    ...
                    stimulations[i, episode, trial] = agent._stimulation_level
    ...
    def train(self):
        ...
        for seed in range(self._num_seeds):
            ...
            stimulations.append(data["stimulations"])
        ...
        self._stimulations = np.swapaxes(np.stack(stimulations), 0, 1)
    ...
    def post_process(self):
        ...
        for array, array_name in [
            ...
            (self._stimulations, constants.STIMULATIONS),
            (self._moment_error, constants.MOMENT_ERROR),
            (self._best_arms, constants.BEST_ARMS),
        ]:
