...
def average_best_arm_plot(agent_order, correct_arm, change_freq, save_path):
    fig = plt.figure()
...
def average_regret_plot(agent_order, regret, change_freq, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(
...
def cumulative_regret(agent_order, regret, save_path):
    ...
    for i, name in enumerate(agent_order):
        ...
        plt.plot(means, lw=2, label=name)
...
def learning_rates(agent_order, learning_rates, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(learning_rates[i].mean(0).ravel(), label=name)
    plt.yscale("log")
    plt.xlabel("Steps")
...
def temperatures(agent_order, temperatures, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(temperatures[i].mean(0).ravel(), label=name)
    plt.yscale("log")
    plt.xlabel("Steps")
...
def min_uncertainties(agent_order, min_uncertainties, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(min_uncertainties[i].mean(0).ravel(), label=name)
    plt.yscale("log")
    plt.xlabel("Steps")
...
def policies(agent_order, policies, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(policies[i].max(-1).mean(0).ravel(), label=name)
    plt.xlabel("Steps")
    plt.ylabel("Policy")
    plt.legend(loc="best")
...
def mean_mses(agent_order, mses, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(mses[i, ..., 0].mean(0).ravel(), label=name)
    plt.xlabel("Steps")
...
def variance_mses(agent_order, mses, save_path):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
        plt.plot(mses[i, ..., 1].mean(0).ravel(), label=name)
    plt.xlabel("Steps")
...
def scalar_plot(agent_order, scalar_data, save_path, label, logscale):
    fig = plt.figure()
    for i, name in enumerate(agent_order):
