...
class EpsilonGreedy(RandomMAB):
    def __init__(self, n_arms, epsilon, optimizer, Q0=0.001, rng=None):
    ...
    def policy(self):
        a = jax.device_get(self.qvals.argmax()).ravel()[0]
        return (1 - self._epsilon) * np.eye(self._n_arms)[a] + (
            self._epsilon / self._n_arms
        )
    ...
    def play(self):
        ...
        if self.rng.random() <= self._epsilon:
            return self.rng.choice(self._n_arms)

        return jax.device_get(self.qvals.argmax()).ravel()[0]
